...
// Thunks
export const fetchProfessionals = createAsyncThunk<Professional[]>('professionals/fetch', async (_, { rejectWithValue }) => {
  try {
...
export const createProfessional = createAsyncThunk<Professional, Omit<Professional, 'id'>>(
...
export const updateProfessional = createAsyncThunk<Professional, Professional>(
  ...
  async (data) => {
    const response = await apiClient.put<Professional>(`/professional/${data.id}`, data);
    return response.data;
  }
);
...
const professionalSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchProfessionals.fulfilled, (state, action: PayloadAction<Professional[]>) => {
      ...
      .addCase(updateProfessional.fulfilled, (state, action: PayloadAction<Professional>) => {
        const index = state.list.findIndex((professional) => professional.id === action.payload.id);
        if (index !== -1) {
      ...
      .addCase(deleteProfessional.fulfilled, (state, action: PayloadAction<string>) => {
        state.list = state.list.filter((professional) => professional.id !== action.payload);
        state.status = 'succeeded';
      })
